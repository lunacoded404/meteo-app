...
export default function TemperaturePopup({
  data,
  loading,
  error,
}: TemperaturePopupProps) {
  const provinceName = data?.province?.name ?? "Không rõ vùng";
  const currentTemp = data?.current?.temperature ?? null;

  // ✅ thời gian cập nhật theo giờ/ngày/tháng/năm
  ...
  // ✅ icon theo nhiệt độ cao/thấp (ngưỡng bạn có thể chỉnh)
  const iconSrc =
    currentTemp != null && currentTemp >= 28 ? "/high_temp.png" : "/short_temp.png";

  // ✅ mô tả nhiệt độ hiện tại
  const currentDesc =
    currentTemp == null
      ? "Chưa có dữ liệu nhiệt độ hiện tại."
      : tempDescriptionVN(currentTemp);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3">
          <div className="flex items-end justify-between gap-3">
            ...
            <div className="flex gap-4">
              <Stat
                label="Max"
                value={today?.tmax == null ? "—" : `${fmt(today.tmax, 0)}°C`}
              />
              <Stat
                label="Min"
                value={today?.tmin == null ? "—" : `${fmt(today.tmin, 0)}°C`}
              />
            </div>
          </div>
