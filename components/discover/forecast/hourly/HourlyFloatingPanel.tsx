...
export default function HourlyFloatingPanel({
  topOffsetPx = 110,
  activeOffsetPx = 120,
  items,
  defaultActiveId = "h-temp",
  onRefresh,
  className,
}: {
  topOffsetPx?: number;
  ...
  const navItems: NavItem[] = useMemo(
    () =>
      items?.length
        ? items
        : [
            { id: "h-temp", label: "Nhiệt độ", icon: <Thermometer className="h-4 w-4" /> },
            { id: "h-humidity", label: "Độ ẩm", icon: <Droplets className="h-4 w-4" /> },
            { id: "h-wind", label: "Gió", icon: <Wind className="h-4 w-4" /> },
            { id: "h-cloud", label: "Mây", icon: <Cloud className="h-4 w-4" /> },
            { id: "h-rain", label: "Mưa", icon: <Umbrella className="h-4 w-4" /> },
          ],
    [items]
  );
