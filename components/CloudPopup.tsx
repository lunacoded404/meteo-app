...
const cloudDescriptionVN = (c: number, visKm: number | null) => {
...
export default function CloudPopup({ data, loading, error }: CloudPopupProps) {
  const provinceName = data?.province?.name ?? "Không rõ vùng";

  const cloud = data?.current?.cloud_cover_percent ?? null;
  const visKm = formatVisibilityKm(data?.current?.visibility_m ?? null);

  const timeText = `Cập nhật lúc: ${formatDateTimeVN(data?.current?.time)}`;

  const desc =
    cloud == null
      ? "Chưa có dữ liệu mây hiện tại."
      : cloudDescriptionVN(cloud, visKm);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3">
          <div className="flex items-end justify-between gap-3">
            ...
            <div className="flex gap-4">
              <Stat
                label="Trạng thái"
                value={cloud == null ? "—" : cloudLabelVN(cloud)}
              />
            </div>
          </div>
        </div>
