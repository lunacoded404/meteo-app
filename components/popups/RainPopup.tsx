...
export default function RainPopup({ data, loading, error, regionName }: RainPopupProps) {
  ...
  return (
    <PopupCard
      icon={<img src="/umbrella.png" alt="" className="w-6 h-6" draggable={false} />}
      title={provinceName}
      timeText={timeText}
      description={desc}
      loading={loading}
                ...
                sub={
                  <span className="inline-flex items-center gap-2">
                    <span className="h-2 w-24 rounded-full bg-slate-200 overflow-hidden">
                      <span className="block h-full bg-slate-900" style={{ width: `${probBar}%` }} />
                    </span>
